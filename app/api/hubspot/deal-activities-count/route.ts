...
export async function GET(request: Request) {
  try {
    ...
    const apiPath = API_CONFIG.getApiPath('/deal-activities-count');
    ...
    console.error('Error fetching deal activities count:', error);
    return NextResponse.json({ error: 'Failed to fetch deal activities count' }, { status: 500 });
  }
} 