...
const ControlPanel: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (shouldFetchData && !loading && isComponentMounted) {
      fetchPipelineData();
    }
  ...
  }, [lastFetched]); // Remove pipelineData.length from dependencies
  ...
  const fetchPipelineData = async () => {
    try {
      ...
      const response = await fetch('/api/hubspot/pipeline-summary');
      if (!response.ok) {
