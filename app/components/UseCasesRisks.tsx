...
const UseCasesRisks: React.FC<UseCasesRisksProps> = ({ browserId, isInitialized }) => {
  ...
  useEffect(() => {
    if (!insights || !isInitialized || !browserId) return;

    const dealNames = new Set<string>();

    // Get deal names from use cases
    if (insights.useCases) {
      selectedStages.forEach(stage => {
        const stageData = insights.useCases[stage];
    ...
    // Get deal names from risks
    if (insights.risks) {
      selectedStages.forEach(stage => {
        const stageData = insights.risks[stage];
  ...
  const handleGetInsights = async () => {
    ...
    try {
      ...
      const fetchPromises = selectedStages.flatMap(async (stage) => {
        ...
        console.log('Fetching use cases from:', useCasesEndpoint);
        console.log('Fetching risks from:', risksEndpoint);

        const [useCasesResponse, risksResponse] = await Promise.all([
          fetch(useCasesEndpoint, {
            headers: {
              'X-Browser-ID': browserId,
              'X-Session-ID': localStorage.getItem('sessionId') || '',
            },
  ...
  const renderCombinedTable = () => {
    if (!insights || selectedStages.length === 0) return null;

    // Collect use cases from all selected stages
    const allUseCases: UseCase[] = [];
    if (insights.useCases) {
      selectedStages.forEach(stage => {
        const stageData = insights.useCases[stage];
        if (Array.isArray(stageData)) {
          allUseCases.push(...stageData);
        }
      });
    }

    // Collect risks from all selected stages
    ...
    // Merge use cases by deal name
    const useCasesByDeal: Record<string, string[]> = {};
    allUseCases.forEach((item: UseCase) => {
      if (!useCasesByDeal[item.deal_name]) {
        useCasesByDeal[item.deal_name] = [];
      }
      if (!useCasesByDeal[item.deal_name].includes(item.use_case)) {
        useCasesByDeal[item.deal_name].push(item.use_case);
      }
    });

    // Merge risks by deal name
    const risksByDeal: Record<string, Array<{ riskType: string; explanation: string }>> = {};
    allRisks.forEach(risk => {
      if (!risksByDeal[risk.dealName]) {
        risksByDeal[risk.dealName] = [];
      }
      risksByDeal[risk.dealName].push({
        riskType: risk.riskType,
        explanation: risk.explanation,
      });
    });

    // Get all unique deal names
    const allDealNames = new Set([
      ...Object.keys(useCasesByDeal),
      ...Object.keys(risksByDeal)
    ]);

    if (allDealNames.size === 0) {
      return (
        <div className="text-center py-12 text-gray-500 dark:text-gray-400">
          No data found for the selected stages and date range.
        </div>
      );
    }

    // Combine into final data structure
    const combinedData = Array.from(allDealNames).map(dealName => ({
      deal_name: dealName,
      use_cases: useCasesByDeal[dealName] || [],
      risks: risksByDeal[dealName] || [],
    }));

    // Filter by selected risk types
    const filteredData = selectedRiskTypes.length > 0
      ? combinedData.map(item => ({
          ...item,
          risks: item.risks.filter(risk => selectedRiskTypes.includes(risk.riskType)),
        })).filter(item => item.risks.length > 0 || item.use_cases.length > 0)
      : combinedData;
    ...
    return (
      <div>
        ...
        {/* Combined Table */}
        <div className="overflow-x-auto max-h-[420px] overflow-y-auto rounded-md border border-gray-100 dark:border-slate-700">
          <table className="min-w-full">
            <thead className="sticky top-0 z-10 bg-white dark:bg-slate-800 border-b border-gray-200 dark:border-slate-600">
              <tr>
                <th className="px-4 py-2 text-left text-[11px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide w-1/5">
                  Deal
                </th>
                <th className="px-4 py-2 text-left text-[11px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide w-1/3">
                  Use Cases
                </th>
                <th className="px-4 py-2 text-left text-[11px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide w-1/3">
                  Risks
                </th>
                <th className="px-4 py-2 text-left text-[11px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide w-24">
                  Owner
                </th>
                <th className="px-4 py-2 text-right text-[11px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide w-20">
                  Last Touch
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50 dark:divide-slate-700/50">
              {filteredData.map((item, index: number) => (
                <tr key={index} className="group hover:bg-gray-100 dark:hover:bg-slate-700 transition-colors">
                  ...
                  <td className="px-4 py-2 text-[13px] text-gray-600 dark:text-gray-400 leading-relaxed align-top">
                    {item.use_cases.length > 0 ? (
                      ...
                      <span className="text-gray-400 dark:text-gray-500 italic">No Use Cases Identified</span>
                    )}
                  </td>
                  <td className="px-4 py-2 text-[13px] text-gray-600 dark:text-gray-400 align-top">
                    {item.risks.length > 0 ? (
                      <div className="space-y-2">
                        {item.risks.map((risk, riskIdx) => (
                          <div key={riskIdx} className="flex flex-col gap-1">
                            ...
                            <p className="text-[12px] text-gray-700 dark:text-gray-300 pl-1">
                              {risk.explanation}
                            </p>
                          </div>
                        ))}
                      ...
                      <span className="text-gray-400 dark:text-gray-500 italic">No Risks Identified</span>
                    )}
                  </td>
          ...
          <div className="flex flex-col items-center justify-center py-16">
            ...
            <div className="mt-8 text-center">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
                Analyzing use cases & risks for {selectedStages.length} stage{selectedStages.length !== 1 ? 's' : ''}
              </p>
            </div>
          </div>
        </div>
